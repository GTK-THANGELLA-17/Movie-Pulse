...
export class ClearStatsExporter {
  static async exportToExcel(data: ClearExportData, filename?: string): Promise<void> {
    try {
      ...
      const overviewData = [
        { 'Report Section': 'Audience-Pulse ANALYTICS OVERVIEW', 'Details': '' },
      ...
      const fileName = filename || `Audience-Pulse_Analytics_${new Date().toISOString().split('T')[0]}.xlsx`;
  ...
  static async exportToWord(data: ClearExportData, filename?: string): Promise<void> {
    try {
      ...
      link.href = url;
      link.download = filename || `Audience-Pulse_Report_${new Date().toISOString().split('T')[0]}.docx`;
  ...
  static async exportToText(data: ClearExportData, filename?: string): Promise<void> {
    try {
      ...
      const link = document.createElement('a');
      link.href = url;
      link.download = filename || `Audience-Pulse_Analysis_${new Date().toISOString().split('T')[0]}.txt`;
      document.body.appendChild(link);
      link.click();
      document.body.removeChild(link);
      URL.revokeObjectURL(url);
...
<head>
    <meta charset="UTF-8">
    <title>Audience-Pulse Analytics Report</title>
    <style>
...
<body>
    <h1>🎬 Audience-Pulse Analytics Report</h1>
    ...
    <div style="margin-top: 40px; padding: 20px; background: #f1f5f9; border-radius: 8px; text-align: center;">
        <p><strong>📈 Generated by Audience-Pulse Analytics Platform</strong></p>
  ...
  private static generateClearTextContent(data: ClearExportData): string {
    let content = `🎬 Audience-Pulse ANALYTICS REPORT\n`;
    content += '='.repeat(50) + '\n\n';
    ...
    content += '\n' + '='.repeat(50) + '\n';
    content += '📈 Generated by Audience-Pulse Analytics Platform\n';
